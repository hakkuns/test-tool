...
import type { TestScenario, UpdateScenarioInput, ApiTestConfig } from '@/types/scenario'
...
export default function ScenarioDetailPage() {
	...
	useEffect(() => {
		const fetchScenario = async () => {
			try {
				...
				setExpectedBody(JSON.stringify(data.expectedResponse?.body || {}, null, 2))
			} catch (error) {
	...
	const handleSave = async () => {
		try {
			...
			const expectedResponseBody = expectedBody.trim() ? JSON.parse(expectedBody) : undefined
			...
			const input: UpdateScenarioInput = {
				...
				tableData,
				mockApis,
				expectedResponse: expectedStatus || expectedResponseBody
					? {
		...
		<div className="container mx-auto p-6 space-y-6 max-w-4xl">
				...
				<CardContent className="space-y-4">
					<div className="grid grid-cols-4 gap-4">
						<div>
							...
							<Select value={apiMethod} onValueChange={(value) => setApiMethod(value as ApiTestConfig['method'])}>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>テーブル定義、データ、モックAPIをJSON形式で指定</CardDescription>
				</CardHeader>
				<CardContent>
			...
			<div className="flex justify-end gap-2">
				<Button variant="outline" onClick={handleApply} disabled={isApplying} size="lg">
					<Play className="h-4 w-4 mr-2" />
